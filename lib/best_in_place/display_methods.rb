  ...
  module DisplayMethods
    ...
    class Renderer < Struct.new(:opts)
      def render_json(object)
        ...
        when :proc
          {:display_as => opts[:method].call(object.send(opts[:attr]))}.to_json
        else
          {}.to_json
        end
      end
    ...
    def add_proc_method(klass, attr, proc_method)
      @@table[klass.to_s][attr.to_s] = Renderer.new :method => proc_method, :type => :proc, :attr => attr
    end
  end
end
